...
const EditForm = ({
  ...
  const [state, formAction, isPending] = useActionState(
    updateRoom.bind(null, image, room.id), null
 );
    ...
    <form action={formAction}>
        <div className="grid md:grid-cols-12 gap-5">
            <div className="col-span-8 bg-white p-4">
                ...
                <div className="mb-4">
                    <textarea name="description" rows={8} defaultValue={room.description} className="py-2 px-4 rounded-sm border border-gray-400 w-full" placeholder="Room Description"></textarea>
                    <div aria-live="polite" aria-atomic="true">
                ...
                <div className="mb-4 grid md:grid-cols-3">
                    ...
                    <label htmlFor="amenities" className="ms-2 text-sm font-medium text-gray-900 capitalize">{item.name}</label>
                    </div>
                    ))}
                    <div aria-live="polite" aria-atomic="true">
            ...
            <div className="col-span-4 bg-white p-4">
                <label htmlFor="input-file" className="flex flex-col mb-4 items-center justify-center aspect-video border-2 border-gray-300 border-dashed rounded-md cursor-pointer bg-gray-50 relative">
                    <div className="flex flex-col items-center justify-center text-gray-500 pt-5 pb-6 z-10">
                        ...
                        <button type="button" onClick={() => deleteImage(image)} className="flex items-center justify-center size-6 rounded-sm absolute right-1 top-1 text-white bg-red-400 cursor-pointer">
                            <IoTrashOutline className="size-4 text-white cursor-pointer" />
                        </button>
                        ) : (
                ...
                <button type="submit" className={clsx("bg-orange-400 text-white w-full hover:bg-orange-500 py-2.5 px-6 md:px-10 text-lg font-semibold cursor-pointer", {"opacity-50 cursor-progress": isPending,})} disabled={isPending}>{isPending ? "Updating..." : "Update"}</button>
            </div>
        </div>
    </form>
    </>
